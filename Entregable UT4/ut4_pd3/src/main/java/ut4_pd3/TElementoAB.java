...
public class TElementoAB<T> implements IElementoAB {
    ...
    public Comparable EtiquetaAnterior(Comparable etiqueta){
        if(getHijoIzq().getEtiqueta().equals(etiqueta))
            return getHijoIzq().getEtiqueta();
        if(getHijoDer().getEtiqueta().equals(etiqueta))
            return getHijoDer().getEtiqueta();
        Comparable res = null;
        if(getHijoIzq() != null){
    ...
    //6
    public boolean ArbolBB(){
        if(getHijoIzq() == null && getHijoDer() == null){
            return true; //Es hoja
        }
        boolean bandera = true;
        if(getHijoIzq() != null){
            if(getHijoIzq().getEtiqueta().compareTo(getEtiqueta()) != -1)
                return false;
        }
        if(getHijoDer() != null){
            if(getHijoDer().getEtiqueta().compareTo(getEtiqueta()) == -1){
                return false;
            }
        }
        if(getHijoIzq() != null){
            bandera = getHijoIzq().ArbolBB();
            if(bandera == false){
                return bandera;
            }
        }
        if(getHijoDer() != null){
            bandera = getHijoDer().ArbolBB();
        }
        return bandera;
    }
